...
# ---------------------------------------------------------------------------------------------------
    def quantizeWeights(self, model=[]):
        """ Quantizes the weights originalWeights into 8bit using min/max
        ...
        originalWeights: this weights will be quantized into Q-format
        """
        ...
        for layer in model.layers:
            ...
            for weights in weightsArray:
                ...
                intWeights[intWeights > 2**self.fixPointBits -
                           1] = 2**self.fixPointBits - 1
                intWeights[intWeights < -2**self.fixPointBits] = - \
                    2**self.fixPointBits
...
# ---------------------------------------------------------------------------------------------------
#                             Functions to find ideal output shifts of ARM layers
# ---------------------------------------------------------------------------------------------------
    def findOutputFormat(self, startBit=0, stopBit=4, model=[], evalData=[]):
        ...
        model:      keras.engine.sequential.Sequential model
        weights:    weights of the model
        startBit:   min number of M bits in Q-format: QM.N
        stopBit:    max number of M bits in Q-format: QM.N
        evalData:   evaluation data to evaluate the model
        """
        ...
        denseLayers = self.getDenseLayersInformation(model)
        ...
        tmpModels[0] = self.buildNewModel(model)
        ...
        for denseLayer in range(0, len(denseLayers)):
            ...
            for intBit in range(startBit, stopBit):
                ...
                evalResult[intBit] = tmpModels[1].evaluate(
                    evalData[0], evalData[1], verbose=0)[1]

            self.bestIntBits[denseLayer] = evalResult.argmax()
            tmpModels[0] = tmpModels[1]
        for denseLayer in range(0, len(denseLayers)):
            print('Dense_'+str(denseLayer) + ' Output range: Q' + str(int(self.bestIntBits[denseLayer])) + '.'
    ...
    def buildNewModelWithAllLambda(self, model, intBits, weights):
        """ Rebuilds model and sets the arguments of the lambda layers
    ...
    @staticmethod
    def getDenseLayersInformation(model):
...
# ---------------------------------------------------------------------------------------------------
        ...
        with open(fp, 'a') as f:
            ...
            for i in range(0, self.numberOfDenselayers - 1):
                ...
                if outShift < 0:
                    ...
                    outShift = 0
                f.write('#define LAYER_' + str(i + 2) +
                        '_IN_SHIFT ' + str(int(inShift)) + '\n')
                f.write('#define LAYER_' + str(i + 2) +
                        '_OU_SHIFT ' + str(int(outShift)) + '\n')
                idx = idx + 2
    ...
    def storeNetFunction(self, model=[]):
        ...
        with open(fp, 'w') as f:
            ...
            if self.fixPointBits == 7:
                f.write(
                    'q15_t aq15_in_Buf[LAYER_1_IN_DIM * LAYER_1_OU_DIM];\n')
